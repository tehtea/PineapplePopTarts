...
/**
 * Testing the getCoor function. 2 equivalence classes - valid postal code and invalid postal code.
 */
...
describe("incidentDataProcessing", function() {
    // scraped this from console.log. First array is incidents array, second array is updates array
    ...
    describe("validData", function() {
        it("should return a response array of the same length as the incidents array provided", function() {
            incidentDataProcessing(validData).then((resp) => {
                expect(resp.length == validData[0].length);
            });
        });
    })
    describe("invalidData", function() {
        var invalidData = [JSON.parse(JSON.stringify(validData[0])), []] // deep clone validData[0]
        invalidData[0][0].Location = 365215;
        it("should throw an error because one of the incidents have invalid postal code", function(done) {
            incidentDataProcessing(invalidData).then((resp) => {
                done(new Error("Should not have successfully finished the function"));
            }).catch((err) => {
                done();
                // expect(err);
            });
